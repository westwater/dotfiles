...
() {
  ...
  # Connect left prompt lines with these symbols (blue style from old theme).
  typeset -g POWERLEVEL9K_MULTILINE_FIRST_PROMPT_PREFIX=$'%F{blue}╭%f%F{blue}\uE0B6%f'
  ...
  # Connect right prompt lines with these symbols (disabled).
  ...
  typeset -g POWERLEVEL9K_LEFT_SUBSEGMENT_SEPARATOR=$'%F{blue}\uE0BC%f'
  ...
  # Shows "SSH user@hostname" with grey background, positioned after dir/vcs.
  function prompt_ssh_indicator() {
    [[ -n "$SSH_CONNECTION" || -n "$SSH_TTY" ]] || return
    p10k segment -f 255 -b 240 -i $'\uF489' -t '%n@%m'
  }
